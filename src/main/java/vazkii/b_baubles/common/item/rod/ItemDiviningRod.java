...
public class ItemDiviningRod extends ItemMod  {
	...
	public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer p) {
			if(world.isRemote) {
				...
				p.swingItem();
			} else world.playSoundAtEntity(p, "b_baubles:divinationRod", 1F, 1F);
				...
				for(int k = -range; k < range + 1; k++) {
					...
					for(int id : OreDictionary.getOreIDs(orestack)) {
						...
						if(s.matches("^ore[A-Z].+")) {
							...
							//																												>these next 3 are RGB colours - use rand.nextFloat() for random
							Botania.proxy.wispFX(world, xp + world.rand.nextFloat(), yp + world.rand.nextFloat(), zp + world.rand.nextFloat(), 30F, 0F, 0F, 0.25F, 0F, 8);
							break;
						}
					}
				}
